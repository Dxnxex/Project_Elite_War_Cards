function sc_card_missions_secure_draw_specific_at_pos(argument0,argument1){
...
if i <= cardMissionSecureNumberMaxAll {
		...
		//-----OBRÁZEK-----
		sc_text_lengdir(380,90); draw_sprite_ext(CardSecure_ImageOut[i],0,xx+(wsprite/2)+lengthdir_x(pos,rot+angle),yy+(hsprite/2)+lengthdir_y(pos,rot+angle),ImageScale,ImageScale,rot,c_white,1);

		//------CARD Layout-----
		draw_sprite_ext(sprite,0,xx+(wsprite/2)+lengthdir_x(0,0),yy+(hsprite/2)+lengthdir_y(0,0),1,1,rot,c_white,1);
	
		//-----Název karty-----
		sc_font(fo_mission_name); sc_text_lengdir(796.50,-269.89); sc_text_align(fa_middle,fa_center); sc_card_text_shadow_less(string_upper(CardMissionSecure_Name[i]),c_black);
	...
	#region MISE (TOKEN & POPIS)
		...
		if CardMissionSecure_DescType[i] !=2 {draw_sprite_ext(CardMissionSecure_TextMainTokenElement[i,Element],0,xx+(wsprite/2)+lengthdir_x(pos,rot+angle),yy+(hsprite/2)+lengthdir_y(pos,rot+angle),tscale,tscale,rot,c_white,1);} else 
		{draw_sprite_ext(CardMissionSecure_TextMainToken[i],0,xx+(wsprite/2)+lengthdir_x(pos,rot+angle),yy+(hsprite/2)+lengthdir_y(pos,rot+angle),tscale,tscale,rot,c_white,1);}
		sc_cirle();
	...
	#region ODMĚNY
		...
		if CardMissionSecure_Desc_2_Check[i] == 1 {draw_sprite_ext(CardMissionSecure_Desc_2_Token[i],0,xx+(wsprite/2)+lengthdir_x(pos,rot+angle),yy+(hsprite/2)+lengthdir_y(pos,rot+angle),tscale,tscale,rot,c_white,1);}		//Element
		if CardMissionSecure_Desc_3_Check[i] == 1 {draw_sprite_ext(CardMissionSecure_Desc_3_Token[i],0,xx+(wsprite/2)+lengthdir_x(pos,rot+angle),yy+(hsprite/2)+lengthdir_y(pos,rot+angle),tscale,tscale,rot,c_white,1)};		//Energy
		sc_cirle();		
		...
		draw_sprite_ext(CardMissionSecure_Desc_1_Token[i],0,xx+(wsprite/2)+lengthdir_x(pos,rot+angle),yy+(hsprite/2)+lengthdir_y(pos,rot+angle),tscale,tscale,rot,c_white,1);
		sc_cirle();
	...
	draw_sprite_ext(CardMissionSecure_Desc_5_Token[i],0,xx+(wsprite/2)+lengthdir_x(pos,rot+angle),yy+(hsprite/2)+lengthdir_y(pos,rot+angle),tscale,tscale,rot,c_white,1);		
	sc_cirle();
