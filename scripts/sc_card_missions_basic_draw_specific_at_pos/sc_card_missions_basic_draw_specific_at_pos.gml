...
if i <= cardMissionBasicNumberMaxAll {
	...
	#region MISE (TOKEN & POPIS)
		...
		sc_card_text_shadow_less(CardMissionBasic_TextMain[i],c_black);
	...
	#region ODMĚNY
		...
		if CardMissionBasic_Desc_2_Check[i] == 1 {draw_sprite_ext(CardMissionBasic_Desc_2_Token[i],0,xx+(wsprite/2)+lengthdir_x(pos,rot+angle),yy+(hsprite/2)+lengthdir_y(pos,rot+angle),tscale,tscale,rot,c_white,1);}		//Element
		if CardMissionBasic_Desc_3_Check[i] == 1 {draw_sprite_ext(CardMissionBasic_Desc_3_Token[i],0,xx+(wsprite/2)+lengthdir_x(pos,rot+angle),yy+(hsprite/2)+lengthdir_y(pos,rot+angle),tscale,tscale,rot,c_white,1)};		//Energy
		sc_cirle();		

		//-----Text Element & Energie-----
		sc_font(fo_mission_reward);	sc_text_align(fa_middle,fa_left);	sc_text_lengdir(402.13,-155.70);
		if CardMissionBasic_Desc_2_Check[i] == 1 {sc_card_text_shadow(CardMissionBasic_Desc_2_Action[i],c_white);};
		if CardMissionBasic_Desc_3_Check[i] == 1 {sc_card_text_shadow(CardMissionBasic_Desc_3_Action[i],c_white);};
		sc_cirle();
		
		//----TEXT - REUNY-----
		sc_font(fo_mission_reward);	sc_text_align(fa_middle,fa_right);	sc_text_lengdir(432.81,-22.62);
		sc_card_text_shadow(CardMissionBasic_Desc_1[i],c_white);

		//----TOKEN - Reuny -----
		sc_text_lengdir(528.10,-18.26);
		draw_sprite_ext(CardMissionBasic_Desc_1_Token[i],0,xx+(wsprite/2)+lengthdir_x(pos,rot+angle),yy+(hsprite/2)+lengthdir_y(pos,rot+angle),tscale,tscale,rot,c_white,1);
		sc_cirle();
	...
	sc_card_text_shadow(string_upper(CardMissionBasic_DescAfterText[i]),c_white);
	...
	sc_card_text_shadow(CardMissionBasic_Desc_5[i],c_white);

	//-----TOKEN AFTER-----
	sc_text_lengdir(572.07,-145.68);
	draw_sprite_ext(CardMissionBasic_Desc_5_Token[i],0,xx+(wsprite/2)+lengthdir_x(pos,rot+angle),yy+(hsprite/2)+lengthdir_y(pos,rot+angle),tscale,tscale,rot,c_white,1);		
	sc_cirle();
	...
	sc_card_text_shadow(CardMissionBasic_Desc_4[i],c_white);			
