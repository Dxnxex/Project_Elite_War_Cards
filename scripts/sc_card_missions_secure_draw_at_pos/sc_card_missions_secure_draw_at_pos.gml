 function sc_card_missions_secure_draw_at_pos(argument0,argument1){
...
if i <= cardMissionSecureNumberMaxAll {
	...
	#region MISE (TOKEN & POPIS)
		...
		draw_sprite_ext(CardMissionSecure_6_Token_1[i],0,xx+(wsprite/2)+lengthdir_x(pos,rot+angle),yy+(hsprite/2)+lengthdir_y(pos,rot+angle),tscale,tscale,rot,c_white,1);
		sc_cirle();	
		
		//-----TOKEN RIGHT-----
		sc_text_lengdir(528.81,-17.44);
		draw_sprite_ext(CardMissionSecure_6_Token_2[i],0,xx+(wsprite/2)+lengthdir_x(pos,rot+angle),yy+(hsprite/2)+lengthdir_y(pos,rot+angle),tscale,tscale,rot,c_white,1);
		sc_cirle();		
