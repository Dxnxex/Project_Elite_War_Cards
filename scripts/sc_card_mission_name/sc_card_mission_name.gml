...
if CardMissionType == 0 {
	...
	CardMissionBasic_Name[cardMissionBasicNumber] = argument0;
		...
		CardBasic_ImageOut[cardMissionBasicNumber] = sprite_add(working_directory+string("sprites/Basic/")+string(Path)+string("."+string(InputFormat)),0,0,1,0,0);
...
if CardMissionType == 1 {
	...
	CardMissionSecure_Name[cardMissionSecureNumber] = argument0;
		...
		var Path = argument2;
		if Path == undefined {Path = "BoardGame - CardsArt - Error"}	
		CardSecure_ImageOut[cardMissionSecureNumber] = sprite_add(working_directory+string("sprites/Secure/")+string(Path)+string("."+string(InputFormat)),0,0,1,0,0);
...
if CardMissionType == 2 {
	...
	CardMissionSabotage_Name[cardMissionSabotageNumber] = argument0;
		...
		var Path = argument2;
		if Path == undefined {Path = "BoardGame - CardsArt - Error"}	
		CardSabotage_ImageOut[cardMissionSabotageNumber] = sprite_add(working_directory+string("sprites/Sabotage/")+string(Path)+string("."+string(InputFormat)),0,0,1,0,0);
