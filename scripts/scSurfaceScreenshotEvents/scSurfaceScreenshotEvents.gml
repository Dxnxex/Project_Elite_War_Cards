...
repeat(_card_event_numbermaxpages) {
	...
	for(var i=0;i<=1;i++) {
			...
			sc_cardback_PDF_pre();
			if (i<1) {sprite_save(spr_custom, 0,string(working_directory+string("Page/")+string(PDF_P_Add)+string(PDF_P)+string("_P."+string(ScreenshotFormat))))		;PDF_P++;sc_cardback_PDF_pre();}
			if (i>=1) {sprite_save(spr_custom, 0,string(working_directory+string("Page/")+string(PDF_Z_Add)+string(PDF_Z)+string("_Z."+string(ScreenshotFormat))))		;PDF_Z++;sc_cardback_PDF_pre();}
