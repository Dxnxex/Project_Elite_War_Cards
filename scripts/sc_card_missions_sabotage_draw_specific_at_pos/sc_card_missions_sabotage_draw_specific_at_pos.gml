...
if i <= sabotageNumberMaxAll {
		...
		//-----Název karty-----
		setFont(fo_mission_name); setLenghtdir(796.50,-269.89); setTextAlign(fa_middle,fa_center); textShadowLess(string_upper(sabotageName[i]),c_black);
	...
	#region ODMĚNY
		...
		if CardMissionSabotage_Desc_2_Check[i] == 1 {draw_sprite_ext(CardMissionSabotage_Desc_2_Token[i],0,posX+(wsprite/2)+lengthdir_x(pos,rot+angle),posY+(hsprite/2)+lengthdir_y(pos,rot+angle),tscale,tscale,rot,c_white,1);}		//Element
		if CardMissionSabotage_Desc_3_Check[i] == 1 {draw_sprite_ext(CardMissionSabotage_Desc_3_Token[i],0,posX+(wsprite/2)+lengthdir_x(pos,rot+angle),posY+(hsprite/2)+lengthdir_y(pos,rot+angle),tscale,tscale,rot,c_white,1)};		//Energy
		sc_cirle();		
		...
		draw_sprite_ext(CardMissionSabotage_Desc_1_Token[i],0,posX+(wsprite/2)+lengthdir_x(pos,rot+angle),posY+(hsprite/2)+lengthdir_y(pos,rot+angle),tscale,tscale,rot,c_white,1);
		sc_cirle();
	...
	draw_sprite_ext(CardMissionSabotage_Desc_5_Token[i],0,posX+(wsprite/2)+lengthdir_x(pos,rot+angle),posY+(hsprite/2)+lengthdir_y(pos,rot+angle),tscale,tscale,rot,c_white,1);		
	sc_cirle();
